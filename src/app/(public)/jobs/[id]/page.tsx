...
import { MapPin, Briefcase, Clock, DollarSign, Users, CheckCircle, BookmarkPlus, ArrowLeft, Building2, Loader2, Calendar, GraduationCap } from 'lucide-react';
            ...
            <div className="bg-white border border-line rounded-[20px] p-6">
              <div className="flex items-start gap-4 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {job.company?.website && (
                    <a href={job.company.website} target="_blank" rel="noopener noreferrer" className="text-sm text-ink-muted hover:text-ink-soft transition-colors">{job.company.website}</a>
                  )}
                </div>
              </div>
                ...
                {(job.minSalary || job.maxSalary) && (
                  <div className="bg-canvas rounded-[16px] p-3">
                    ...
                    <p className="text-sm text-ink font-medium">
                      {job.minSalary ? `${(job.minSalary/100000).toFixed(0)}L` : ''}{job.maxSalary ? `-${(job.maxSalary/100000).toFixed(0)}L` : ''} PA
                    </p>
                  </div>
                )}
              </div>
