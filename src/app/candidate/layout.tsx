...
export default function CandidateLayout({ children }: { children: React.ReactNode }) {
  ...
  const [account, setAccount] = useState<{ name: string; email: string; image: string | null } | null>(
    null
  );

  const onboarding = pathname === '/candidate/onboarding';

  // A candidate who never finished the wizard has no usable dashboard, so send
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 shadow-e4 animate-slide-in">
            <Sidebar mobile pathname={pathname} onNavigate={() => setMobileOpen(false)} onLogout={logout} />
          </div>
        </div>
      )}
            ...
            <Link href="/candidate/profile" aria-label="My profile">
              {account?.image ? (
                ...
                <img
                  src={account.image}
                  alt=""
                  className="w-9 h-9 rounded-[11px] object-cover border border-line"
                />
              ) : (
