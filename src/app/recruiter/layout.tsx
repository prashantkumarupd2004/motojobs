...
import {
  LayoutDashboard, Building2, PlusCircle, Briefcase, Search, Database,
  Kanban, Star, Users, Wand2, LogOut, ChevronLeft, ChevronRight, Menu, Gauge
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { href: '/recruiter/candidate-search', icon: Search, label: 'Find Candidates' },
  { href: '/recruiter/resume-database', icon: Database, label: 'Resume Database' },
  { href: '/recruiter/ats', icon: Kanban, label: 'ATS Board' },
  { href: '/recruiter/ratings', icon: Star, label: 'Ratings' },
  { href: '/recruiter/bulk-hiring', icon: Users, label: 'Bulk Hiring' },
  { href: '/recruiter/ai-tools', icon: Wand2, label: 'AI Tools' },
];
    ...
    >
      ...
      <div className={`flex items-center h-[72px] px-4 border-b border-line-soft shrink-0 ${collapsed && !mobile ? 'justify-center' : 'justify-between'}`}>
        {(!collapsed || mobile) ? (
          <Link href="/" className="flex items-center gap-2.5 group">
            <div className="relative w-9 h-9 rounded-[12px] grad-ignite flex items-center justify-center shadow-[0_4px_10px_rgba(255,107,0,0.24)] transition-transform duration-500 [transition-timing-function:cubic-bezier(0.22,1,0.36,1)] group-hover:-translate-y-0.5">
            ...
            <div className="leading-none">
              ...
              <span className="block text-[8.5px] font-bold tracking-[0.2em] text-ink-faint uppercase mt-1">Recruiter</span>
            </div>
          </Link>
        ) : (
          <div className="relative w-9 h-9 rounded-[12px] grad-ignite flex items-center justify-center shadow-[0_4px_10px_rgba(255,107,0,0.24)]">
            <Gauge className="w-[17px] h-[17px] text-white" strokeWidth={2.2} />
          </div>
        )}
        {!mobile && !collapsed && (
          <button
            ...
            className="text-ink-faint hover:text-ignite-600 hover:bg-ignite-50 p-1.5 rounded-[10px] transition-all duration-200"
      ...
      {collapsed && !mobile && (
        <button
          ...
          className="mx-auto mt-3 text-ink-faint hover:text-ignite-600 hover:bg-ignite-50 p-1.5 rounded-[10px] transition-all duration-200"
      ...
      {(!collapsed || mobile) && (
        <div className="px-4 pt-4">
          <span className="inline-flex items-center gap-1.5 text-[10px] bg-ignite-50 text-ignite-700 border border-ignite-100 rounded-full px-3 py-1.5 font-bold uppercase tracking-[0.12em]">
      ...
      {(!collapsed || mobile) && user && (
        <div className="px-4 py-4 shrink-0">
          <div className="flex items-center gap-3 bg-canvas border border-line-soft rounded-[16px] p-3">
            <div className="w-10 h-10 rounded-[12px] grad-ignite flex items-center justify-center text-white font-bold text-sm shrink-0 shadow-[0_3px_8px_rgba(255,107,0,0.22)]">
              {user.name[0]}
            </div>
          ...
          return (
            <Link
              ...
              className={`relative flex items-center gap-3 px-3 py-2.5 rounded-[13px] transition-all duration-300 [transition-timing-function:cubic-bezier(0.22,1,0.36,1)] group ${
                active
                  ? 'bg-ignite-50 text-ignite-700 font-semibold shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]'
                  : 'text-ink-muted hover:text-ignite-600 hover:bg-canvas'
    ...
    <div className="flex h-screen bg-canvas overflow-hidden">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 animate-slide-in"><Sidebar mobile /></div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-[72px] glass-nav border-b border-line flex items-center justify-between px-5 sm:px-7 shrink-0 z-10">
          <button
            className="lg:hidden text-ink-soft hover:text-ignite-600 hover:bg-ignite-50 p-2.5 rounded-[12px] transition-all duration-200"
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="sweep press inline-flex items-center gap-2 grad-ignite text-white text-[13.5px] font-semibold px-5 py-2.5 rounded-[12px] shadow-[0_4px_12px_rgba(255,107,0,0.22)] hover:shadow-[0_8px_20px_rgba(255,107,0,0.30)] hover:-translate-y-0.5 transition-all duration-300 [transition-timing-function:cubic-bezier(0.22,1,0.36,1)]"
            >
              <PlusCircle className="w-4 h-4" /> Post Job
            </Link>
            {user && (
              <div className="w-9 h-9 rounded-[12px] grad-ignite flex items-center justify-center text-white text-[13px] font-bold shadow-[0_3px_8px_rgba(255,107,0,0.22)]">
                {user.name[0]}
              </div>
            )}
          </div>
